...
def createObjectMods(objects, config):
	mods = list()
	#modZPos = ShuffleZPos([-0.8, -0.1], objects, True, False)
	...
	for obj in objects:
		...
		#erzeuge Zufallszahl zwischen 0 und 0,5
		rand_num= random.uniform(0.3, 0.7)
		modZPos = ShuffleZPos([-rand_num-0.03, -rand_num+0.05], obj_list, True, False)
		mods.append(modZPos)
		
		x = 0.1+ 0.2*rand_num 
		modXPos = ShuffleXPos([-x, x], obj_list, True, False)
		mods.append(modXPos)
		modYPos = ShuffleYPos([-x, x], obj_list, True, False)
		mods.append(modYPos)
