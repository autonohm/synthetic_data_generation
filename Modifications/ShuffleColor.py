...
class ShuffleRGBColor(ShuffleColor):
	...
	def Action(self, obj):
		#print(nodes.get(self.node_name).inputs)
		...
		if self.property_name == "":
			if self.mode == "rgb":
				nodes.get(self.node_name).inputs[0].default_value = srgb2lin(val1), srgb2lin(val2), srgb2lin(val3), 1
			if self.mode == "hsv":
				col = colorsys.hsv_to_rgb(val1, val2, val3)
				nodes.get(self.node_name).inputs[0].default_value = srgb2lin(col[0]), srgb2lin(col[1]), srgb2lin(col[2]), 1
			else:
